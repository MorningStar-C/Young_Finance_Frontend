    ...
    <ul class="preferred-select clearfix">
      <li v-for="(product, index) in productList" :key="product.id">
        <h3 class="preferred-select-title">
          ...
          <img src="@/assets/image/1-bg2.jpg" alt="" v-if="index % 4 == 2">
          <img src="@/assets/image/1-bg3.jpg" alt="" v-if="index % 4 == 3">
          <img src="@/assets/image/1-bg4.jpg" alt="" v-if="index % 4 == 0">
        </h3>
...
export default {
  ...
  watch: {
    getName: {
      handler() {
        this.initPage(this.$route.query.ptype,1,9)
      },
    }
  },
  ...
  methods: {
    ...
    first() {
      if(this.page.pageNo == 1) {
        // alert("已经是第一页数据")
        layx.msg("已经是第一页数据", {dialogIcon: 'warn', position: 'tc'})
      } else {
        this.initPage(productType,1,9)
      }
    },
    ...
    last() {
      if(this.page.pageNo == this.page.totalPage) {
        // alert("已经是第一页数据")
        layx.msg("已经是最后一页数据", {dialogIcon: 'warn', position: 'tc'})
      } else {
